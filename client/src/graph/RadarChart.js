...
class RadarChart extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    this.getRadarData(nextProps);
  }

  getRadarData(props) {
    console.log('componentWillReceiveProps');
    var playerId = props.playerId;
    console.log('re mount', playerId);
    const reqObj = {
      filters: {
        playerId,
      },
    ...
    .then(data => {
      console.log('set state for new radar chart');
  ...
  render() {
    console.log('re render RadarChart');
    if (this.state.data) {

      console.log('should load every click');
      const RadarChart = {
        draw: (id, d, options) => {
          const cfg = {
            radius: 5,
      ...
      for (const stat in topStats) {
        const statValue = (this.state.data[0][0][stat] * 100) / topStats[stat];
...
function mapStateToProps(state) {
  ...
  var playerId;
  if (state.modal[0]) {
    playerId = state.modal[0][0][0].playerId;
